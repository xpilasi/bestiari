<script>

import MobileFeatureBg from '@/assets/img/design/mobile-cool.png'
export default {
  name: 'MobileFeatureWidget',
  data() {
    return {
      MobileFeatureBg: MobileFeatureBg
    }
  },
  props: {
    number: {
      type: String,
      required: true
    },
    title: {
      type: String,
      required: true
    },
    subtitle: {
      type: String,
      required: true
    },
    numberColor: {
      type: String,
      default: 'text-[#E61655]'
    },
    titleColor: {
      type: String,
      default: 'text-white'
    },
    subtitleColor: {
      type: String,
      default: 'text-white'
    },
    numberSize: {
      type: String,
      default: 'text-2xl'
    },
    titleSize: {
      type: String,
      default: 'text-base'
    },
    subtitleSize: {
      type: String,
      default: 'text-xs'
    },
    maxWidth: {
      type: String,
      default: 'max-w-40'
    },
    spacing: {
      type: String,
      default: 'space-y-4'
    }
  }
}
</script>

<template>
    <div class="relative rounded-2xl bg-gradient-to-br from-black to-blue-900 py-8 mx-4 overflow-hidden h-64">
        <!-- Background Image -->
        <!-- <img :src="MobileFeatureBg" alt="Mobile Feature Background" class="absolute inset-0 w-full h-full object-cover"> -->
        
        <!-- Overlay -->
        <div class="absolute inset-0 backdrop-blur-sm bg-white/0"></div> 
        
        <!-- Content Container -->
        <div :class="`relative z-10 px-6 h-full flex flex-col justify-center ${spacing}`">
            <!-- Feature Number -->
            <div :class="`${numberColor} ${numberSize} font-normal leading-none tracking-[2px] font-instrument-serif italic`">
                {{ number }}
            </div>
            
            <!-- Feature Title -->
            <h3 :class="`${titleColor} ${titleSize} font-semibold leading-tight tracking-[-0.06em]`"
                style="font-family: Inter;">
                {{ title }}
            </h3>
            
            <!-- Feature Subtitle -->
            <p :class="`${subtitleColor} ${subtitleSize} font-extralight leading-relaxed tracking-[-0.06em]`"
               style="font-family: Inter;">
                {{ subtitle }}
            </p>
        </div>
    </div>
</template>

<style scoped>
/* Ensure fonts are available */
@import url('https://fonts.googleapis.com/css2?family=Inter:wght@200;400;500;600&display=swap');
</style> 